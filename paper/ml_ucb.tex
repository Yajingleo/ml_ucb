...
adaptations of UCB to machine learning contexts, such as LinUCB
\cite{chu2011contextual} and NeuralUCB \cite{zhou2020neural}, are tailored to
specific scenarios and lack a general framework.
...
In this section, let's recall the $\psi$-UCB framework using Cumulant Generating 
Functions (CGF). The CGF of a random variable $X$ is defined
as $\psi_X(\lambda) = \log \mathbb{E}[e^{\lambda (X - \mathbb{E}[X])}]$. The
Fenchel-Legendre transform of $\psi_X$, denoted by $\psi_X^*$, plays a crucial
role in concentration inequalities.

We define the $\psi$-UCB score as:
$$ \ucb_{j, t} = \hat{E}_{j, T_j(t-1)} + (\psi_{\hat{E}_{j, T_j(t-1)}}^*)^{-1}(3\log{t}). $$
This formulation generalizes the classical UCB by allowing for different tail
behaviors of the estimator distribution.

\section{Calibrate Concentration Inequality for ML-model}

Here we discuss how to calibrate the concentration inequality for machine
learning models by analyzing their learning curves. We assume that the Mean
Squared Error (MSE) of the estimator converges as $O(n^{-s})$ for some $s > 0$.
This assumption allows us to derive a specific form of the $\psi$-function for
ML-based estimators.

Specifically, if the estimator $\hat{E}$ satisfies the generalized $\psi$-growth
condition, we can bound the deviation of the estimator from the true mean using
the learning curve parameters. This leads to the specific UCB formula used in
our algorithm.
...
We apply ML-UCB to collaborative filtering on a simulated dataset. Using
stochastic gradient descent (SGD), we embed users and items into $\mathbb{R}^{10}$
